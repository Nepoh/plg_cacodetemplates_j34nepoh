 ...
 * 
 * @version			$Id: emailcloak.php 482 2015-04-06 17:48:54Z BrianWade $
 ...
 *
 */
class Plg[%%ArchitectComp%%]Emailcloak extends JPlugin
{
[%%FOREACH COMPONENT_OBJECT%%]
	...
	public function on[%%CompObject%%]Prepare($context, &$row, &$params, $page = 0)
	{
		...
		$mode = (int) $this->params->def('[%%compobject%%]_mode', 1);

		if (is_object($row))
		{
			[%%IF INCLUDE_INTRO%%]
			$this->_cloak($row->introdescription, $mode);
			$this->_cloak($row->intro, $mode);			
			[%%ELSE INCLUDE_INTRO%%]
