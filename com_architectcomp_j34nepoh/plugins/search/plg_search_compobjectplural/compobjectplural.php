		...
		if ($s[%%CompObject%%] AND $limit > 0)
		{
			...
			if (isset($list))
			{
				foreach($list as $key => $item)
				{
					[%%IF GENERATE_CATEGORIES%%]		 
						[%%IF INCLUDE_LANGUAGE%%]
					$list[$key]->href = JRoute::_([%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $item->language, $this->layout, $keep_item_id));
						[%%ELSE INCLUDE_LANGUAGE%%]
					$list[$key]->href = JRoute::_([%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $this->layout, $keep_item_id));
						[%%ENDIF INCLUDE_LANGUAGE%%]
					[%%ELSE GENERATE_CATEGORIES%%]
						[%%IF INCLUDE_LANGUAGE%%]
