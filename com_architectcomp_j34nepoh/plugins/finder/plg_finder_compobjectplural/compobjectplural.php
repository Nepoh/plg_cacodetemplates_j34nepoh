...
class PlgFinder[%%CompObjectPlural%%] extends FinderIndexerAdapter
{
	...
	protected function index(FinderIndexerResult $item, $format = 'html')
	{
		...
		[%%IF GENERATE_CATEGORIES%%]		 
			[%%IF INCLUDE_LANGUAGE%%]
		$item->route = [%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $item->language, $this->sub_layout);
			[%%ELSE INCLUDE_LANGUAGE%%]
		$item->route = [%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $this->sub_layout);
