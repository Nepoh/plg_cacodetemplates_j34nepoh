			...
			<?php $class = ''; ?>
				[%%FOREACH COMPONENT_OBJECT%%]
					[%%IF GENERATE_CATEGORIES%%]
						[%%IF INCLUDE_LANGUAGE%%]
				<span class="item-title"><a href="<?php echo JRoute::_([%%ArchitectComp%%]HelperRoute::getCategoryRoute($child->id, $this->params->get('keep_[%%compobject%%]_itemid'), $child->language));?>">
						[%%ELSE INCLUDE_LANGUAGE%%]
				<span class="item-title"><a href="<?php echo JRoute::_([%%ArchitectComp%%]HelperRoute::getCategoryRoute($child->id, $this->params->get('keep_[%%compobject%%]_itemid')));?>">
