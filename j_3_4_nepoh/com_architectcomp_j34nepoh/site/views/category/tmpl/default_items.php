 ...
 *
 * @version			$Id: default_items.php 482 2015-04-06 17:48:54Z BrianWade $
...
<?php else : ?>
<div class="[%%compobjectplural%%]-list">
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_[%%compobject%%]_filter_field') != '' AND $this->params->get('show_[%%compobject%%]_filter_field') != 'hide') OR $this->params->get('show_[%%compobject%%]_pagination_limit')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_[%%compobject%%]_filter_field') != '' AND $this->params->get('show_[%%compobject%%]_filter_field') != 'hide') :?>
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('[%%COM_ARCHITECTCOMP%%]_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('[%%COM_ARCHITECTCOMP%%]_'.$this->params->get('show_[%%compobject%%]_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>						
				...
				[%%ENDIF INCLUDE_STATUS%%]
					...
					[%%ENDIF INCLUDE_ACCESS%%]
						...
						<td class="createdby">
							...
							<?php if (!empty($item->created_by ) AND  $this->params->get('link_[%%compobject%%]_created_by') == 1):?>
								<?php $creator = JHtml::_(
										'link',
