 ...
 *
 * @version			$Id: compobjecticon.php 482 2015-04-06 17:48:54Z BrianWade $
...
class JHTML[%%CompObject%%]Icon
{
	 ...
	 *
	 * @param	Registry   $params	  The [%%compobject%%] parameters
		...
		if ($params->get('show_[%%compobject%%]_icons'))
		{
			if ($legacy)
			{
				...
				if (strtotime($[%%compobject_code_name%%]->publish_up) > strtotime(JFactory::getDate())
					...
					AND $[%%compobject_code_name%%]->publish_down != JFactory::getDbo()->getNullDate()))
				{
			...
			else
			{
				...
				$icon	=  'edit';
				[%%ENDIF INCLUDE_STATUS%%]				
				$text = '<span class="hasTooltip icon-' . $icon . ' tip" title="' . JHtml::tooltipText(JText::_('[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_EDIT_ITEM'), $overlib, 0) 
