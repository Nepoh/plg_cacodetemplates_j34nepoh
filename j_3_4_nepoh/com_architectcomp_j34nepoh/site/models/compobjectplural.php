 ...
 *
 * @version			$Id: compobjectplural.php 486 2015-04-09 13:47:05Z BrianWade $
...
class [%%ArchitectComp%%]Model[%%CompObjectPlural%%] extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('[%%compobject_code_name%%]_id', '') == '' ? $app->input->getInt('id', '') : $params->get('[%%compobject_code_name%%]_id');
	...
	protected function getStoreId($id = '')
	{
		...
		[%%ENDFOR FILTER_FIELD%%]	
		$id .= ':'.serialize($this->getState('filter.[%%compobject_code_name%%]_id'));
	...
	protected function getListQuery()
	{
		...
		$can_publish = $user->authorise('core.edit.state', '[%%com_architectcomp%%]');
		...
		if (!$can_publish)
		{
			...
			[%%ELSE INCLUDE_CREATED%%]			
			$query->select($db->quoteName('a.publish_up').' AS publish_up');
			[%%ENDIF INCLUDE_CREATED%%]	
		...
		$query->join('LEFT', $db->quoteName('#__categories').' AS [%%FIELD_CODE_NAME%%] ON '.$db->quoteName('[%%FIELD_CODE_NAME%%].id').' = '.$db->quoteName('a.[%%FIELD_CODE_NAME%%]'));			
			...
			[%%IF FIELD_USER%%]
		// Join over the users.
		$query->select($db->quoteName('[%%FIELD_CODE_NAME%%].name').' AS [%%FIELD_CODE_NAME%%]_name');
		$query->join('LEFT', $db->quoteName('#__users').' AS [%%FIELD_CODE_NAME%%] ON '.$db->quoteName('[%%FIELD_CODE_NAME%%].id').' = '.$db->quoteName('a.[%%FIELD_CODE_NAME%%]'));		
			[%%ENDIF FIELD_USER%%]		
			[%%IF FIELD_USERGROUP%%]
		...
		if ((is_object($params)) AND ($params->get('show_[%%compobject%%]_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_[%%compobject%%]_filter_field'))
			{
				[%%IF INCLUDE_HITS%%]
				case 'hits':
		...
		[%%IF INCLUDE_ORDERING%%]
			...
			if ($order_col == '')
			{
				$order_col = is_string($this->getState('list.ordering')) ? $this->getState('list.ordering') : $db->quoteName('a.ordering');
		...
		[%%ELSE INCLUDE_ORDERING%%]
			...
			if ($order_col == '')
			{
				$order_col = is_string($this->getState('list.ordering')) ? $this->getState('list.ordering') : $db->quoteName('a.name');
				$order_col .= ' '.$order_dirn;
			}
			[%%ELSE INCLUDE_NAME%%]
			if ($order_col == '')
			{
				$order_col = is_string($this->getState('list.ordering')) ? $this->getState('list.ordering') : $db->quoteName('a.id');
				$order_col .= ' '.$order_dirn;
			}
			[%%ENDIF INCLUDE_NAME%%]
		[%%ENDIF INCLUDE_ORDERING%%]		
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$[%%compobject_code_name%%]_params = new Registry;
