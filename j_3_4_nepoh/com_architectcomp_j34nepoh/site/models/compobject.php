...
class [%%ArchitectComp%%]Model[%%CompObject%%] extends JModelItem
{
	...
	protected function canDelete($record)
	{
		...
		[%%IF INCLUDE_CREATED%%]
			...
			return $user->authorise('core.delete', '[%%com_architectcomp%%].[%%compobject%%].'.(int) $record->id)
					  OR ($user->authorise('core.delete.own', '[%%com_architectcomp%%].[%%compobject%%].'.(int) $record->id)
					  AND $record->created_by == $user->id);
			[%%ELSE INCLUDE_ASSETACL_RECORD%%]
			return $user->authorise('core.delete', '[%%com_architectcomp%%]')
					  OR ($user->authorise('core.delete.own', '[%%com_architectcomp%%]')
					  AND $record->created_by == $user->id);
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the [%%compobject_name%%].
	 *
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				[%%IF INCLUDE_ASSETACL%%]
					[%%IF INCLUDE_ASSETACL_RECORD%%]
						[%%IF INCLUDE_STATUS%%]
				$can_publish = $user->authorise('core.edit.state', '[%%com_architectcomp%%].[%%compobject%%].'.$pk);
						[%%ELSE INCLUDE_STATUS%%]
