 ...
 * 
 * @version			$Id: helper.php 482 2015-04-06 17:48:54Z BrianWade $
		...
		foreach ($items as &$item)
		{
			...
			[%%ENDIF INCLUDE_ACCESS%%]
				if ($item_id_str == '')
				{
					...
					[%%IF GENERATE_CATEGORIES%%]		 
						[%%IF INCLUDE_LANGUAGE%%]
					$item->link = JRoute::_([%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $item->language, $layout));
						[%%ELSE INCLUDE_LANGUAGE%%]
					$item->link = JRoute::_([%%ArchitectComp%%]HelperRoute::get[%%CompObject%%]Route($item->slug, $item->catid, $layout));
						[%%ENDIF INCLUDE_LANGUAGE%%]
