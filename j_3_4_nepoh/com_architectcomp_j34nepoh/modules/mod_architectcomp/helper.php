		...
		foreach ($items as &$item)
		{
			...
			[%%ENDIF INCLUDE_ACCESS%%]
				...
				{			
					...
					{
						$item->link = JRoute::_([%%ArchitectComp%%]HelperRoute::$route_function($item->slug, $item->catid, $item->language));
					}
					else
					{
						if (isset($item->catid))
						{
							$item->link = JRoute::_([%%ArchitectComp%%]HelperRoute::$route_function($item->slug, $item->catid));
						}
						else
						{
