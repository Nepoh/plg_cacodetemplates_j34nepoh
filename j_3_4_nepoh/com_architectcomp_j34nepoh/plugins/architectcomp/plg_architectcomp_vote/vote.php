 ...
 * 
 * @version			$Id: vote.php 482 2015-04-06 17:48:54Z BrianWade $
...
[%%ENDFOR COMPONENT_OBJECT%%]
	...
	protected function OutputRating($context, &$row, &$params, $page=0, $object)
	{
		...
		if ($object <> '')
		{
			...
			$html .= '<p class="unseen element-invisible">'.JText::sprintf( 'PLG_[%%ARCHITECTCOMP%%]_VOTE_USER_RATING', '<span itemprop="ratingValue">'.$rating.'</span>', '<span itemprop="bestRating">5</span>' )
					. '<meta itemprop="ratingCount" content="' . $rating_count . '" />'
					. '<meta itemprop="worstRating" content="0" />'
					. '</p>';
			[%%ELSE INCLUDE_MICRODATA%%]
			$html .= '<div class="'.$object.'_rating">';
			$html .= '<p class="unseen element-invisible">'.JText::sprintf( 'PLG_[%%ARCHITECTCOMP%%]_VOTE_USER_RATING', $rating, '5' ).'</p>';
